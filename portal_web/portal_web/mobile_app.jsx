...
const timeRangeStr = من ${start12h} إلى ${end12h};
      ...
      try {
        ...
        if (fbClientsRes.ok) {
          ...
          if (fbClientsRaw) {
            ...
            const idx = clientList.findIndex(c => c && (
              c.id === client.id ||
  ...
  const confirmCancelBooking = async () => {
    ...
    setMyBookings(prev => (prev || []).map(item => item && item.id === bookingId ? { ...item, status: 'cancelled' } : item));
    setAllBookings(prev => (prev || []).map(item => item && item.id === bookingId ? { ...item, status: 'cancelled' } : item).filter(item => item && item.id !== bookingId));
      ...
      try {
        ...
        if (fbRes.ok) {
          ...
          if (Array.isArray(rawBookings)) {
            const updated = rawBookings.map(item => (item && item.id === bookingId) ? { ...item, status: 'cancelled' } : item);
